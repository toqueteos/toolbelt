...
func TestSet(t *testing.T) {
	...
	require.ElementsMatch(t, []int{1}, slices.Collect(s1.Items()))
	...
	require.ElementsMatch(t, []int{1, 2, 3}, slices.Collect(s1.Items()))
	require.Equal(t, true, s1.Has(1))
	...
	require.ElementsMatch(t, []int{2, 3}, slices.Collect(s1.Items()))
	...
	require.ElementsMatch(t, []int{}, slices.Collect(s1.Items()))
	require.Equal(t, false, s1.Has(1))
	require.Equal(t, false, s1.Has(2))
	require.Equal(t, false, s1.Has(3))

	s2 := NewFrom([]int{1, 1, 1})
	require.Equal(t, 1, s2.Count())
	require.ElementsMatch(t, []int{1}, slices.Collect(s2.Items()))
}
